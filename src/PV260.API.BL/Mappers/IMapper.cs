...
public interface IMapper<TEntity, out TListModel, TDetailModel>
{
    ...
    /// <summary>
    /// Converts a collection of entities to a collection of detail model representations.
    /// </summary>
    /// <param name="entities">The collection of entities to convert.</param>
    /// <returns>A collection of detail model representations.</returns>
    IEnumerable<TDetailModel> ToDetailModel(IEnumerable<TEntity> entities)
        => entities.Select(ToDetailModel);
    
    /// <summary>
    /// Converts a queryable collection of entities to a queryable collection of detail model representations.
    /// </summary>
    /// <param name="entities">The queryable collection of entities to convert.</param>
    /// <returns>A queryable collection of detail model representations.</returns>
    IQueryable<TDetailModel> ToDetailModel(IQueryable<TEntity> entities)
        => entities.Select(entity => ToDetailModel(entity));
