    ...
    where TEntity : class, IEntity
{
    ...
    public virtual async Task<TDetailModel?> GetAsync(Guid id)
    {
        ...
        var entity = await uow.GetRepository<TEntity>().GetByIdAsync(id);
    ...
    /// <inheritdoc />
    public virtual async Task<IQueryable<TListModel>> GetQueryAsync()
    {
        await using var uow = UnitOfWorkFactory.Create();
        var entities = uow.GetRepository<TEntity>().Get();
